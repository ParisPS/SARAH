...
- **Achado real, fora do escopo desta fase**: nesse mesmo teste, uma
  ...
  `docs/architecture.md`.
- **Escopo original também incluía reserva de restaurante/ingresso de
  museu** — pesquisado (OpenTable/Resy/Reserve with Google não têm API
  de reserva acessível pra projeto pessoal; Tiqets/GetYourGuide têm
  elegibilidade incerta pra uso pessoal e a parte de COMPRA exige
  volume comercial mínimo) e uma arquitetura foi proposta (Google
  Places API + tools que preparam a reserva/visita pro usuário
  completar, nunca reserva de verdade) — mas **descartada por decisão
  consciente do usuário**, não por falta de caminho técnico nem por
  ficar pra depois. Escopo final da fase é só `web.search_price`.
